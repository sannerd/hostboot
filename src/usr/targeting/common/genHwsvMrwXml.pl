...
my $chipIds = XMLin($chip_ids_file, forcearray=>['chip-id']);
...
my $dmibus = XMLin($dmi_busses_file, forcearray=>['dmi-bus']);
...
my $fsiBus = XMLin($fsi_busses_file, forcearray=>['fsi-bus']);
...
if ($haveFSPs)
{
    ...
    $psiBus = XMLin($psi_busses_file, forcearray=>['psi-bus']);
